...
/**
 * {@code ParticleManager.random} places the particles that are spawned when a block is broken.
 */
@Mixin(ParticleManager.class)
public class MixinParticleManagerRand {
    @ModifyExpressionValue(method = "<init>", at = @At(value = "NEW", target = "Ljava/util/Random;"))
    public Random modifyRandom(Random original) {
        return RandomManager.create(RandomTypes.PARTICLE_MANAGER);
    }
}
